export const actions = [
  ...
  { id: 'toggleDiffView', label: 'Toggle inline/side-by-side diff view', defaultBinding: 'Alt+KeyV' },
...
export function defaultBindings() { return Object.fromEntries(actions.map(({ id, defaultBinding }) => [id, defaultBinding])); }
...
export function presetForBindings(bindings) {
...
export function mergeBindings(value) {
...
export function matchesEvent(binding, event) { return Boolean(binding) && !event.repeat && !event.isComposing && bindingForEvent(event) === normalizeBinding(binding); }
...
export function assignBinding(bindings, actionID, binding) {
...
export function createShortcutCoach({
...
// One shared instance, same singleton semantics as the former
// `globalThis.GoLensShortcutCoach`: every importer of this module resolves
// to the same module instance, so they all get the same coach.
export const shortcutCoach = createShortcutCoach();
