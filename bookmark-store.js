...
export const KEY_PREFIX = `golensBookmark:v${VERSION}:`;
...
export function sameMergeRequest(left, right) {
...
export function createStore({ storage = globalThis.chrome?.storage?.local, storageChanges = globalThis.chrome?.storage?.onChanged, now = () => Date.now(), id = makeID } = {}) {
