...
| --- | --- | --- | --- |
| `shortcut-settings.js` | Define portable shortcut actions, GoLens/VS Code/IntelliJ/Vim-style presets, normalization, display labels, matching, and duplicate reassignment. | `chrome.storage.sync.shortcutBindings`, platform modifier conventions | Invalid stored entries fall back to defaults; explicitly cleared actions stay unassigned; presets remain editable after application. |
