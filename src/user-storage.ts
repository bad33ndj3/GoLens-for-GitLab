...
declare const __GOLENS_ACTIVE_ARCHITECTURE_EPOCH__: number | null;
export const ACTIVE_ARCHITECTURE_EPOCH: number | null = typeof __GOLENS_ACTIVE_ARCHITECTURE_EPOCH__ === 'undefined'
  ? null : __GOLENS_ACTIVE_ARCHITECTURE_EPOCH__;
...
type ResetStorageArea = Readonly<{
  get(keys?: string | readonly string[] | Record<string, unknown> | null): Promise<Record<string, unknown>>;
  set(values: Record<string, unknown>): Promise<void>;
  clear(): Promise<void>;
}>;
...
export function createStorageResetCoordinator({ epoch, sync, local, clearCache }: {
  epoch: number | null;
  sync: ResetStorageArea;
  local: ResetStorageArea;
...
type ResetRuntime = Pick<typeof chrome.runtime, 'sendMessage'>;

export async function ensureStorageReady(runtime: ResetRuntime = chrome.runtime): Promise<StorageResetState> {
...
export async function acknowledgeUpgradeNotice(runtime: ResetRuntime = chrome.runtime): Promise<void> {
