...
- `content.js` detects GitLab merge requests, mounts the four-button AI-sidebar control strip, owns focus mode and file-search shortcuts, and renders first-run onboarding.
- `bookmark-store.js` validates and persists minimal versioned MR-local bookmark records in extension-local storage.
