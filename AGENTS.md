...
The six page controls, from top to bottom, turn GoLens on or off, enter or leave fullscreen review focus, cache related Go packages for the MR head, open MR-local bookmarks, switch GitLab's diff between inline and side-by-side view (driven through GitLab's own preferences control, configurable shortcut default `Alt+V`), and copy accepted review comments as a paste-ready agent bundle. Overview code discussions expose an **Accept** action that includes the reviewer's exact request, including suggested changes, plus optional local guidance. Writing guidance accepts the thread automatically, and the compact action changes from **Guidance** to **Guided** while text is saved. The extension stores the batch in `chrome.storage.local`, never sends it to GitLab, and restores it after refresh or navigation. `golensReview:v1:` records own the acceptance flag and optional guidance for each note. Hover a Go identifier for its signature and documentation. Plain-click selects its loaded-diff occurrences; Cmd-click on macOS or Ctrl-click elsewhere resolves definitions, usages, or interface implementations, and `Cmd/Ctrl+F12` performs the same action for the selected occurrence by default. Configurable shortcuts move between occurrences, hunks, files, bookmarks, and in-diff semantic history; settings can apply GoLens, VS Code, IntelliJ IDEA, or non-modal Vim-style keymaps before editing individual actions. Contextual shortcut tips may teach a configured binding after the equivalent manual action, with local learning state, a session and time cooldown, and a synced enablement preference. `Cmd/Ctrl+P` focuses GitLab's file search and `Shift+F` clears it by default. The compact toolbar popup owns global enablement, active-project cache status, full-project caching, and the settings entry point. Its gear opens the large tabbed `settings.html` extension iframe inside `#golens-settings-root`; that surface owns review preferences, shortcuts, self-hosted origin approval, cache management, and tour replay.
