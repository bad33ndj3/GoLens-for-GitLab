...
The three page controls, from top to bottom, turn GoLens on or off, enter or leave fullscreen review focus, and cache related Go packages for the MR head. Hover a Go identifier for its signature and documentation. Cmd-click on macOS or Ctrl-click elsewhere resolves definitions, usages, or interface implementations. `Cmd/Ctrl+P` focuses GitLab's file search and `Shift+F` clears it. The toolbar popup is the separate control plane for the global switch, full-project caching, cache size and clearing, and replaying the quick tour.
